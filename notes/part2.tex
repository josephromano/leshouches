...
limit.
For concreteness, we will consider the LIGO Hanford-LIGO Livingston
detector pair and the LIGO-Hanford-Virgo detector pair.
...
For the LIGO Hanford-LIGO Livingston detector pair,
...
don't lie in same plane.
The two interferometers are separated by $27.2^\circ$ as 
seen from the center of the Earth, so the tangent planes are
tilted relative to one another due to the curvature of
the Earth.
...
equal to 
twice the separation (3000~km) between the two observatories.
For lower frequencies the two interferometer are driven
(on average) by the same positive (or negative) part of
a passing GW; for slightly larger frequencies, the two 
interferometers will start to be driven by parts of the GW 
...
For the LIGO Hanford-Virgo interferometer pair, note 
that in the limit $f\rightarrow 0$, $\gamma(f)\approx 0$.
This means that the two interferometers are effectively
orthogonal to one another---i.e., rotated by $45^\circ$.
The reduced amplitude of the LHO
