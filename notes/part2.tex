...
\noindent
1) They begin by splitting data in short (e.g., 4 sec) segments, 
which should contain at most one BBH merger.

\smallskip
\noindent
2) They choose a mixture signal model:
%
\be 
p(h|\xi, \vec\lambda) 
...
\smallskip
\noindent
3) For each segment of data $d_i$, they marginalize
...
%
\be
p(d_i|\xi, \vec\lambda, C_n)=\int {\rm d_i}h\>
...
\smallskip
\noindent
4) They marginalize over the BBH signal
...
\smallskip
\noindent
5) They calculate the posterior for each segment
using Bayes theorem
%
\be
p(\xi|d_i)
...
\smallskip
\noindent
6) They finally combine segments
...
\be
\frac{{\rm SNR}_{\rm non-stationary}}{{\rm SNR}_{\rm stationary}}\sim \sqrt{\frac{N_{\rm cycles}}{\xi}}
\ee
...
Although as of summer 2019 we have not yet detected such a signal, 
we know that such a signal exists and its just a matter of time 
...
Then again, pulsar timing arrays might make the first 
detection of a stochastic background, although this will be 
for the inspiraling SMBHs in the centers of distant galaxies.
...
But unlike the detection of the individual large signal-to-noise 
ratio BBH and BNS mergers, we won't be able to say early on that 
we've detected the
...
