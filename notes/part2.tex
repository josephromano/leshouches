...
parameters: $S_{n_1}$ and $S_{n_2}$ for the detector noise, 
and $S_h$ for the GWB.
...
and signal+noise models, one can construct the 
...
$\hat S_{n_1}$, $\hat S_{n_2}$, $\hat S_h$, can be used as 
frequentist estimators of the true values of the parameters 
$S_{n_1}$, $S_{n_2}$, $S_h$.
In Exercise~\ref{exer:8} you are asked to show that 
%
\be
\hat S_h \equiv 
...
\ee
% 
which follow from \eqref{e:stochastic_likelihood} and 
\eqref{e:covariance_matrices}.
Note that the maximum-likelihood estimator $\hat S_h$ 
is just the standard cross-correlation estimator
$\hat C_{12}$ introduced in \eqref{e:Sh_ML}.
...
\ee
%
where the approximate equality is appropriate for the
{\em weak-signal approximation}, where we assume that 
$S_h\ll S_{n_1}, S_{n_2}$.
The quantity $\Lambda(d)$ can be used as a frequentist detection 
statistic, by comparing its value for the given data $d$ 
to a threshold $\Lambda_*$.
If $\Lambda(d)\ge \Lambda_*$, we reject the null 
hypothesis (that is, the noise-only model) and claim detection 
