...
In other words, the measured value of the 
...
the simple single-component optimal-filtering model, 
to a signal model that allows for a superpostion 
...
We will treat $\hat C_{12}(f)$ for different frequencies 
...
$\hat C_{12}(f)$ 
given the amplitudes of the GWB spectral components 
$A_\alpha$ and the noise in the two detectors $N_{12}(f,f')$.

We are now in a position To obtain estimators of the 
amplitudes of the components of the GWB, by maximizing
the likelihood function with respect the $A_\alpha$.
The final result, as you will show in Exercise~\ref{exer:5}(b),
is 
%
\be
\hat A = F^{-1} X\,,
...
\ee
%
The quantity $F$ is called the {\em Fisher information matrix}.
It is basically a noise-weighted inner product 
of the spectral shapes $H_\alpha(f)$, $H_\beta(f)$ together with
the overlap function $\Gamma_{12}(f)$.  
...
Using the above formalism, we are now able to extract the 
ampltitude of the white GWB component to 7.3\%, 
...
In short, the {\em joint} multicomponent analysis 
