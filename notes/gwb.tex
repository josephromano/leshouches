...
\subsection{Ensemble averages and the energy 
density spectrum in GWs}
...
zero---i.e., $\langle h_A(f,\hat n)\rangle=0$.
The quadratic (or second-order) moments 
specify possible correlations between the Fourier
coefficients.
For example, for an unpolarized, stationary, 
isotropic background, we have
%
\be
...
\ee
%
where $S_h(f)$ is the strain power spectral
density of the background.
(The fact that the RHS is proportional 
to $\delta(f-f')$ is a consequence of stationarity.
That it is proportional to $\delta_{AA'}$ is a 
consequence of the background being unpolarized---i.e.,
that the $+$ and $\times$ polarization components 
are statistically
equivalent and uncorrelated with one another.
And since the RHS depends on the directions $\hat n$,
$\hat n'$ only via $\delta(n,n')$ is a consequence 
of isotropy.
For an unpolarized, stationary, anisotropic background

For {\em Gaussian} backgrounds, all cubic 
and higher-order moments are either identically zero 
or can be written in terms of the quadratic moments.
Thus, the quadratic expectation values of the Fourier
coefficients completely characterize the statistical
properties of a Gaussian-distributed GWB.
