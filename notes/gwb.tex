...
the CMB blackbody radiation, relative 
...
We also expect that there are many more signals, 
...
background, we can estimate the total rate of stellar-mass
BBH mergers throughout the universe using the 
local rate estimate from these first detections,
...
combined signal is highly-nonstationary (or {\em popcorn}-like).
...
Thus, the signals overlap in time leading to a 
continuous (or {\em confusion-limited}) background.
...
\begin{figure}[htbp!]
\begin{center}
\includegraphics[width=0.5\textwidth]{Figures/BBH-BNS-timeseries}
...
Figure 3-$\sigma$, correspond to a 
