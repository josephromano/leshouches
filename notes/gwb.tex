...
We will do this in Sections~\ref{s:nontrivial_response} and
\ref{s:nontrivial_correlations}.
For now, we will simply state without proof that the 
...
$\Gamma_{12}(f)$, which relates the strain power in the
...
For reference, the (auto-correlated) power spectra 
...
\ee
A plot of $\Gamma_{12}(f)$ for the LIGO Hanford and
LIGO Livingston interferometers are given in 
Section~\ref{s:nontrivial_correlations}.

Given the above definitions, we can now ask and answer
the question: what is the optimal way to correlate the 
data from two physically separated and possibly 
mis-aligned detectors to search for an isotropic GWB?
To do this, we start by forming the generic 
cross-correlation
%
\be
...
version of the Dirac delta function defined as
$\delta_T(f-f')\equiv T\,{\rm sinc}\,(f-f')$, where
${\rm sinc}\,x \equiv \sin x/x$.

Our criterion for optimality is to chose the filter
function $Q$ so that the expected signal-to-noise 
ratio of $\hat C_{12}$ is maximized.
