...
The quadratic (i.e., second-order) moments 
specify possible correlations between the Fourier
coefficients.
For example, for an {\em unpolarized, stationary, 
...
density} of the background, with units of ${\rm Hz}^{-1}$.
The fact that the RHS is proportional 
to $\delta(f-f')$ is a consequence of stationarity,
which is the lack of a preferred origin of time.
...
Similarly. the dependence on sky directions
only via $\delta(n,n')$ is a consequence of isotropy.
