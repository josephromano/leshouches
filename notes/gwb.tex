...
searches is to produce the GW equivalent of Figure~\ref{f:CMB},
%
\begin{figure}[htbp!]
\begin{center}
...
compared to the electromagnetic force, the GW equivalent 
...
of light.
In so-called transverse-traceless coordinates
...
we take $\hat l$, $\hat m$ to be the 
standard angular unit vectors tangent to the sphere:
%
\be
\begin{aligned}
\hat n
...
Since the metric perturbations $h_{ab}(t,\vec x)$ 
for a stochastic background are random variables, 
so too are the 
Fourier coefficients $h_A(f,\hat n)$.
The probability distributions of $h_A(f,\hat n)$ 
define the statistical properties of the background.
