...
That is, the total cross-correlated power in the GWB is
...
{\em optimal}.
A natural criterion in this context is to {\em maximize 
the expected signal-to-noise ratio} of $\hat C_{12}$ 
for a GWB with fixed spectral shape $H(f)$.
...
We now apply the above correlation methods to some simulated data.%
...
We will consider three different GWB injected into 
...
of the GWB backgrounds from (i) and (ii).
The three cases are shown in Figure~\ref{f:simple_examples}.
Recall that a white GWB has spectral shape $H(f)=1$, while
a confusion-limited background of BNS has spectral shape
$H(f)=(f/f_{\rm ref})^{-7/3}$ (Figure~\ref{f:different_power_spectra}).
