...
This means that although the GWB might be weak 
...
a single sample of data from two detectors:
%
\be
\begin{aligned}
d_1 &= h + n_1\,,
...
noise components.
Cross-correlating the data for this case simply 
amounts to taking the product of the two data 
...
\ee
which is just the variance (or power) in the GW signal.

\subsection{Extension to multiple data samples}

The above analysis can be easily extended to multiple samples.
Here we will consider the case of $N$ sample of a {\em white}
GWB in {\em white} uncorrelated detector noise:
%
\be
\begin{aligned}
d_{1i} &= h_i + n_{1i}\,,
...
%
where $i=1,2,\cdots,N$.
The statement that the GWB and signal are ``white" means
%
\be
